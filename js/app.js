...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('negotiation-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('negotiation-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('negotiation-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('negotiation-progress', JSON.stringify(this.progress));
        localStorage.setItem('negotiation-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    showDashboard() {
        ...
        var html = '<div class="fade-in">' +
            ...
            '<h2>交渉術 Academy へようこそ</h2>' +
            '<p>ハーバード流交渉術からWin-Win戦略まで、交渉スキルを体系的に学べるプラットフォームです。</p>' +
            '</div>' +
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('negotiation-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('negotiation-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('negotiation-progress');
            localStorage.removeItem('negotiation-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
